...
// 1. Replace the entire <head> up to <style> and body styling
...
html = html.replace(/<!DOCTYPE html>[\s\S]*?<div id="root"><\/div>/i, newHead);

// 2. Replace the Sidebar & App layout wrapper
const oldAppWrapperRegex = /return \([\s\S]*?<div className="flex h-screen bg-gray-50 text-gray-900">[\s\S]*?{renderContent\(\)}\s*<\/main>\s*<\/div>\s*<\/div>\s*\);/i;
...
html = html.replace(/return \([\s\S]*?<div className="flex h-screen bg-gray-50 text-gray-900">[\s\S]*?{renderContent\(\)}\s*<\/main>\s*<\/div>\s*<\/div>\s*\);/i, newAppWrapper);

// 3. Global CSS Class Replacements in Components
// We will replace legacy tailwind classes with the new unified glass-card & btn-primary classes
const replacements = [
    // Dashboard Kpis
    ...
    [/bg-green-600 text-white font-bold py-3 rounded hover:bg-green-700/g, "btn-primary w-full py-4"],
    ...
    [/<\/h3>\s*<table/g, '</h3></div><table'], // fixes the div wrap around the table header
...
// Write completed logic back
fs.writeFileSync('inventory.html', html);
console.log('UI Patch Applied Completely.');
