...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ALIASES_KEY = 'inh_usa_aliases_v1';
    ...
    const CONFIG_KEY = 'inh_usa_config_v1';
    ...
    async function loadData() {
        ...
        try {
            ...
            rawHeaders.forEach((h, idx) => {
                ...
                if (hLow.includes('name') || hLow.includes('salesman') || hLow.includes('salesperson') || hLow.includes('delegate')) guessedSalesmanIdx = idx;
            });
    ...
    function extractSalesmen() {
        ...
        if (uniqueSalesmen.size === 0) {
            ...
            salesmanFilter.innerHTML = '<option value="">-- All Names --</option>';
            Array.from(uniqueSalesmen).sort().forEach(s => {
                salesmanFilter.innerHTML += `<option value="${s}">${s}</option>`;
            });
            
            if (uniqueSalesmen.has('INH-USA')) {
                salesmanFilter.value = 'INH-USA';
            } else if (uniqueSalesmen.has('INH USA')) {
                salesmanFilter.value = 'INH USA';
            }
        }
